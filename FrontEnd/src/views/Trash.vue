<template>
  <div class="trash">
    <Navbar :username="username" navigation="Trash" />
                ...
                <v-dialog v-model="removeTagDialogs[index]" width="800">
                  ...
                  <v-card>
                    <v-card-title class="grey lighten-2">
                      ...
                      <div
                        class="caption"
                        v-text="`<${project.mailHeader.Sender}>`"
                      ></div>
                    </v-card-title>
...
export default {
  ...
  async mounted() {
    await this.autoDelete()
    await this.sortGazzar('Date')
    //this.loadMails()
  },
  methods: {
    async autoDelete(){
      await axios.post("http://localhost:8085/autoDelete", JSON.stringify(new User(this.username, null, null, null)), {
        headers: {
